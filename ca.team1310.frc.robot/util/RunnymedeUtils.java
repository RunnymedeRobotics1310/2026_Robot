package ca.team1310.frc.robot.util;
...
public class RunnymedeUtils {
    ...
    public static DriverStation.Alliance getRunnymedeAlliance() {
        ...
        if (
            alliance == null ||
            currentTime - allianceLastUpdated > ALLIANCE_CACHE_TIME_MILLIS
        ) {
            DriverStation.getAlliance().ifPresent(value -> {
                alliance = value;
     ...
     */
    public static Translation2d getRedAllianceLocation(
        Translation2d blueAllianceTranslation
    ) {
        return new Translation2d(
            Constants.FieldConstants.FIELD_EXTENT_METRES_X -
                blueAllianceTranslation.getX(),
            Constants.FieldConstants.FIELD_EXTENT_METRES_Y -
                blueAllianceTranslation.getY()
        );
    }
    ...
    public static Pose2d getRedAlliancePose(Pose2d blueAlliancePose) {
        return new Pose2d(
            Constants.FieldConstants.FIELD_EXTENT_METRES_X -
                blueAlliancePose.getX(),
            Constants.FieldConstants.FIELD_EXTENT_METRES_Y -
                -blueAlliancePose.getY(),
            Rotation2d.fromDegrees(
                SwerveUtils.normalizeDegrees(
                    blueAlliancePose.getRotation().getDegrees() + 180
                )
            )
        );
    }
