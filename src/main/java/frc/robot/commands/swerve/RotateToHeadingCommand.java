...
@AutoConfigurable(value = "rotate", category = "drive",
...
public class RotateToHeadingCommand extends LoggingCommand {
    ...
    public RotateToHeadingCommand(
            ...
            @ConfigParam(value = "headingDegrees", unit = "deg",
                description = "Target heading") double heading,
    ...
    public boolean isFinished() {
        ...
        if (stableNow) {
            ...
            if (Timer.getFPGATimestamp() - stableSinceSeconds >= STABLE_TIME_SECONDS) {
                setFinishReason("Heading settled: " + format(currentHeading)
                        + " deg err=" + format(error));
                return true;
            }
        } else {
            stableSinceSeconds = -1;
